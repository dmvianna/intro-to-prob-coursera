...
sfo_feb_flights %>% summarise(max_delay_h = max(arr_delay)/60, median = median(arr_delay))
```
...
<li> <b>Delta and United Airlines </b></li>
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(arr_delay_iqr = IQR(arr_delay), median(arr_delay)) %>%
  arrange(desc(arr_delay_iqr))
```
...
nycflights <- nycflights %>% 
  mutate(arr_type = if_else(arr_delay <= 0, "on time", "delayed"))
nycflights %>%
  filter(dep_delay > 0) %>%
  summarise(prop_delayed = sum(if_else(arr_type == "on time", 1, 0)) / n())
```